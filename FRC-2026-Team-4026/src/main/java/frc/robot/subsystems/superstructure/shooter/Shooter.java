...
public class Shooter extends SubsystemBase {
    ...
    private final SysIdRoutine sysIdRoutine = new SysIdRoutine(
            new SysIdRoutine.Config(Volts.of(0.2).per(Second), Volts.of(1.5), Seconds.of(10),
                    (state) -> SignalLogger.writeString("state", state.toString())),
            new SysIdRoutine.Mechanism((volts) -> io.setVoltage(volts.in(Volts)), null, this));

 /**
  * Allows change in velocity without re-deploying code, by using a slider in shuffleboard
  * @param io
  */
    ...
    public Command shootAimCommand() {
        return Commands.runOnce(() -> io.setVelocity(ShotEstimator.getInstance().getTargetVelocity().get()));
    }

    public Command shootOnMoveCommand(Drive drive) {
        return Commands.run(() -> io.setVelocity(ShotEstimator.getInstance()
                .getTargetVelocity(drive.getDistanceToHub(drive.convergentFlightTime().get()).getAsDouble()).get()));
    }
